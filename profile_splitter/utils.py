...
import sys
import gc
from profile_splitter.constants import MIN_FILE_SIZE, EXTENSIONS, VALID_INT_TYPES
...
def process_partitions(file_path,format="text"):
    '''
    Reads in a file in (text, parquet) formats and returns a dict of sample_id to partiton
    ...
    return dict(zip(df.index.values.tolist(),df[col].values.tolist(),))
